...
try:
    ...
    Slow(osint_banner)
    email = input(f"{BEFORE_CYAN + current_time_hour() + AFTER_CYAN} {INPUT} Email -> {reset}")
    Censored(email)
    print(f"{BEFORE_CYAN + current_time_hour() + AFTER_CYAN} {WAIT} Scanning..")

    sites = [
        Instagram, Twitter, Pinterest, Imgur, Patreon, Spotify, FireFox, LastPass, Archive
    ]
    ...
    for site in sites:
        ...
        if result:
            ...
            print(f"{BEFORE_CYAN + current_time_hour() + AFTER_CYAN} {GEN_INVALID} {site.__name__}: {white}Not Found")
            not_found += 1

    if found:
        print(f"\n{BEFORE_CYAN + current_time_hour() + AFTER_CYAN} {INFO} Total Found ({white + str(found) + cyan}): {white}" + ", ".join(site_founds))
    print(f"{BEFORE_CYAN + current_time_hour() + AFTER_CYAN} {INFO} Not Found: {white + str(not_found) + cyan} Unknown: {white + str(unknown) + cyan}")
    Continue()
    Reset()
except Exception as e:
    Error(e)